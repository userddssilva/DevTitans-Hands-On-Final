...
import com.android.systemui.notificationdebug.NotificationCollector;
import com.android.systemui.notificationdebug.NotificationSummaryGenerator;
import com.android.systemui.notificationdebug.NotificationSummaryRequest;
import com.android.systemui.notificationdebug.NotificationSummaryResponse;
import com.android.systemui.notificationdebug.NotificationSummaryService;
...
/**
 * Versão modificada do NotificationListener para debug e processamento de notificações.
 * Esta classe coleta todas as notificações, envia para um serviço mockado para resumo,
 * e gera uma nova notificação com o resumo.
 * 
 * MUDANÇAS PRINCIPAIS:
 * 1. Adicionado NotificationCollector para coletar notificações por app
 * 2. Adicionado NotificationSummaryService para resumir notificações
 * 3. Adicionado NotificationSummaryGenerator para gerar notificação resumida
 * 4. Adicionados logs detalhados para debug no logcat
 * 5. Processamento assíncrono do resumo de notificações
 */
@SysUISingleton
...
public class NotificationListener extends NotificationListenerWithPlugins implements
    ...
    private static final long SUMMARY_DEBOUNCE_MS = 2000L; // Aguardar 2s antes de gerar resumo
    ...
    // NOVOS CAMPOS PARA DEBUG E PROCESSAMENTO
    private final NotificationCollector mNotificationCollector;
    private final NotificationSummaryService mSummaryService;
    private final NotificationSummaryGenerator mSummaryGenerator;
    private final Runnable mProcessSummaryRunnable = this::processNotificationSummary;
    private long mLastSummaryRequestTime = 0;
    ...
    public NotificationListener(
        ...
        mNotificationCollector = new NotificationCollector();
        mSummaryService = new NotificationSummaryService();
        mSummaryGenerator = new NotificationSummaryGenerator(context, notificationManager);
        
        if (DEBUG) Log.d(TAG, "NotificationListener initialized with debug features");
    }
    ...
    public void onListenerConnected() {
        ...
        if (DEBUG) Log.d(TAG, "onListenerConnected: Found " + notifications.length + " active notifications");
        ...
        mMainExecutor.execute(() -> {
            ...
            for (StatusBarNotification sbn : notifications) {
                ...
                // NOVO: Coletar notificações para debug
                mNotificationCollector.addNotification(sbn);
                if (DEBUG) Log.d(TAG, "Collected notification on connect: " + sbn.getKey() 
                        + " from " + sbn.getPackageName());
            }
            ...
            // NOVO: Processar resumo após inicialização
    ...
    public void onNotificationPosted(final StatusBarNotification sbn,
        ...
        if (DEBUG && sbn != null) {
            ...
            Log.d(TAG, "  User: " + sbn.getUser().getIdentifier());
        }
        ...
        if (sbn != null && !onPluginNotificationPosted(sbn, rankingMap)) {
            mMainExecutor.execute(() -> {
                ...
                // NOVO: Coletar notificação para processamento
                mNotificationCollector.addNotification(sbn);
                if (DEBUG) Log.d(TAG, "Total notifications collected: " + mNotificationCollector.getTotalNotificationCount()
                        + " from " + mNotificationCollector.getAppCount() + " apps");

                for (NotificationHandler handler : mNotificationHandlers) {
                    handler.onNotificationPosted(sbn, rankingMap);
                }
                
                // NOVO: Agendar processamento de resumo com debounce
                scheduleSummaryProcessing();
            });
        }
    }
    ...
    public void onNotificationRemoved(StatusBarNotification sbn, RankingMap rankingMap,
        ...
        if (sbn != null && !onPluginNotificationRemoved(sbn, rankingMap)) {
            mMainExecutor.execute(() -> {
                // NOVO: Remover notificação do coletor
                mNotificationCollector.removeNotification(sbn);
                if (DEBUG) Log.d(TAG, "Removed notification. Remaining: " + mNotificationCollector.getTotalNotificationCount()
                        + " from " + mNotificationCollector.getAppCount() + " apps");
                ...
                // NOVO: Agendar processamento de resumo após remoção
                scheduleSummaryProcessing();
            });
        }
    }
    ...
    /**
     * NOVO MÉTODO: Agenda o processamento do resumo com debounce.
     */
    private void scheduleSummaryProcessing() {
        mLastSummaryRequestTime = mSystemClock.elapsedRealtime();
        mMainExecutor.execute(() -> {
            try {
                Thread.sleep(SUMMARY_DEBOUNCE_MS);
                // Verificar se ainda é o último request
                if (mSystemClock.elapsedRealtime() - mLastSummaryRequestTime >= SUMMARY_DEBOUNCE_MS) {
                    processNotificationSummary();
                }
            } catch (InterruptedException e) {
                Log.e(TAG, "Error in scheduleSummaryProcessing", e);
            }
        });
    }

    /**
     * NOVO MÉTODO: Processa o resumo de todas as notificações coletadas.
     */
    private void processNotificationSummary() {
        if (DEBUG) Log.d(TAG, "=== PROCESSANDO RESUMO DE NOTIFICAÇÕES ===");
        if (DEBUG) Log.d(TAG, "Total de apps: " + mNotificationCollector.getAppCount());
        if (DEBUG) Log.d(TAG, "Total de notificações: " + mNotificationCollector.getTotalNotificationCount());
        
        if (mNotificationCollector.getTotalNotificationCount() == 0) {
            if (DEBUG) Log.d(TAG, "Nenhuma notificação para processar");
            return;
        }

        // Criar request com todas as notificações
        NotificationSummaryRequest request = mNotificationCollector.createSummaryRequest();
        
        if (DEBUG) Log.d(TAG, "Enviando request para serviço de resumo...");
        
        // Chamar serviço mockado para resumir
        mSummaryService.summarizeNotifications(request)
                .thenAccept(response -> {
                    if (DEBUG) {
                        Log.d(TAG, "=== RESPOSTA DO SERVIÇO ===");
                        Log.d(TAG, "Título: " + response.getTitle());
                        Log.d(TAG, "Resumo: " + response.getSummaryText());
                        Log.d(TAG, "Total de notificações: " + response.getTotalNotifications());
                        Log.d(TAG, "Total de apps: " + response.getTotalApps());
                    }
                    
                    // Gerar nova notificação com o resumo
                    mMainExecutor.execute(() -> {
                        mSummaryGenerator.generateSummaryNotification(response);
                        if (DEBUG) Log.d(TAG, "Notificação resumida gerada e exibida");
                    });
                })
                .exceptionally(throwable -> {
                    Log.e(TAG, "Erro ao processar resumo de notificações", throwable);
                    return null;
                });
    }
